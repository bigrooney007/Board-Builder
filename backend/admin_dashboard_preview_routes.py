...
def create_admin_dashboard_preview_router(db) -> APIRouter:
    ...
    async def seed_fundraising_preview(member: dict) -> None:
        ...
        # A preview reseed must replace the previous internal fixture. The hashed fixture tag
        # protects product isolation, but old internal rows must not remain visible as duplicate
        # Board Members after the fixture version or tag changes.
        old_members = await db.game_board_members.find(
            {"user_id": member["user_id"], "internal_preview": True}, {"_id": 0, "member_id": 1}
        ).to_list(500)
        old_member_ids = [row.get("member_id") for row in old_members if row.get("member_id")]
        old_sessions = await db.group_game_sessions.find(
            {"user_id": member["user_id"], "internal_preview": True}, {"_id": 0, "session_id": 1}
        ).to_list(100)
        old_session_ids = [row.get("session_id") for row in old_sessions if row.get("session_id")]
        old_portfolios = await db.board_portfolios.find(
            {"user_id": member["user_id"], "internal_preview": True}, {"_id": 0, "portfolio_id": 1}
        ).to_list(500)
        old_portfolio_ids = [row.get("portfolio_id") for row in old_portfolios if row.get("portfolio_id")]
        for collection in (
            "game_board_members", "game_section_responses", "game_audience_responses", "game_nights",
            "group_game_sessions", "group_game_participants", "group_game_ideas", "group_game_rounds",
            "group_game_decisions", "game_strategies", "meeting_review_sessions", "final_board_approvals",
            "board_portfolios", "game_strategy_deliveries", "game_relationship_deliveries", "game_relationships",
        ):
            await db[collection].delete_many({"user_id": member["user_id"], "internal_preview": True})
        if old_member_ids:
            for collection in ("game_section_responses", "game_audience_responses", "group_game_participants", "game_relationships"):
                await db[collection].delete_many({"board_member_id": {"$in": old_member_ids}, "internal_preview": True})
        if old_session_ids:
            for collection in ("group_game_participants", "group_game_ideas", "group_game_rounds", "group_game_decisions"):
                await db[collection].delete_many({"session_id": {"$in": old_session_ids}, "internal_preview": True})
        if old_portfolio_ids:
            await db.execution_toolkits.delete_many({"portfolio_id": {"$in": old_portfolio_ids}, "internal_preview": True})
        ...
        for board_member_id, spec in portfolio_specs.items():
            ...
            await db.board_portfolios.update_one(
                ...
                {"$set": {
                    ...
                    "token": portfolio_token, "status": "approved", "version": 1, "approved_version": 1,
                    "system_roles": system_roles, "direct_activities": direct_activities,
            ...
            await db.execution_toolkits.delete_many({"portfolio_id": portfolio_id, "internal_preview": True})
