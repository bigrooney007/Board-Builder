...
1. Produce strengthened_text — a clearer, more specific, actionable, practical, realistic and executable version of that answer. Ask internally: WHO exactly? WHERE exactly? WHY this audience? HOW exactly? What does the person responsible do next? What process needs to run consistently?
...
SECTION_TASKS = {
    ...
    4: "This answer describes how the organization will raise money. Do not simply repeat KNOW, LIKE, TRUST, ASK, FOLLOW UP, STEWARD — turn the stages into an actual operating fundraising process, different where appropriate for individuals, businesses and grantors, describing what actually happens at each stage. In strengthened_text organize under KNOW, LIKE, TRUST, ASK, FOLLOW UP and STEWARD where useful. Each entry is one clear, actionable process statement.",
}
