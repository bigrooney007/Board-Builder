...
import { HOMEPAGE_KEYS, STRATEGIC_PLANNING_SECTION_VIDEO_KEYS, RECOMMITMENT_SECTION_VIDEO_KEYS, RECRUITMENT_SECTION_VIDEO_KEYS, VIDEO_KEYS } from "@/clean/platform";
...
function StrategicPlanningSectionVideosAdmin(){
  ...
  const load=useCallback(async()=>{
    ...
    }catch(error){setMessage(error.response?.data?.detail||"Could not load Strategic Planning section videos.");}
  },[]);
  useEffect(()=>{load()},[load]);
  const save=async(key)=>{
    ...
    try{
      ...
      await load();
    }catch(error){setMessage(error.response?.data?.detail||"Could not save this Strategic Planning section video.");}
    setSaving("");
  };
  const ordered=STRATEGIC_PLANNING_SECTION_VIDEO_KEYS.map(([key,name])=>videos.find(row=>row.key===key)||{key,name,url:""});
  return <div className="admin-platform-subsection" data-testid="admin-strategic-planning-section-videos">
    <div className="admin-funnel-numbers-head" style={{marginTop:36}}>
      <div><h2>Strategic Planning Dashboard Section Videos</h2><p>Paste one YouTube URL for each of the seven Strategic Planning sections. The customer-facing Play Section Video button activates automatically.</p></div>
