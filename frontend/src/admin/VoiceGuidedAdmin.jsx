...
export const VoiceGuidedAdmin = () => {
  ...
  const generateAllMissing = async () => {
    ...
    try {
      ...
      if (response.data.started) { setMessage(`Generating ${response.data.total} missing or updated production clips…`); pollBulk(); }
      else setMessage("All production narration is current.");
      ...
      {open && settings && (
        <div style={{ marginTop: 12 }}>
          ...
          <div style={{ marginTop: 22, padding: 12, background: "#eef2ff", borderRadius: 8 }}>
            ...
            <p style={{ fontSize: 13, marginTop: 4 }}>{missingLive.length} production clip{missingLive.length === 1 ? "" : "s"} need generation. This includes missing audio and audio whose script has changed.</p>
            {bulk?.running ? (
              ...
              <div style={{ marginTop: 8 }} data-testid="voice-bulk-confirm">
                ...
                <p style={{ fontSize: 13, marginTop: 4 }}>You are about to generate {missingLive.length} missing or updated production narration clips using ElevenLabs.</p>
                <p style={{ fontSize: 13, marginTop: 4 }}>Current completed narration will not be regenerated.</p>
                <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                  <button className="button button-small" onClick={() => setBulkConfirm(false)} data-testid="voice-bulk-cancel">Cancel</button>
                  <button className="button button-small" onClick={generateAllMissing} data-testid="voice-bulk-generate">Generate {missingLive.length} Required Clips</button>
                </div>
              </div>
            ) : (
              <button className="button button-small" style={{ marginTop: 6 }} disabled={!missingLive.length}
                onClick={() => setBulkConfirm(true)} data-testid="voice-bulk-open">Generate Missing or Updated Narration</button>
            )}
          </div>

          <h4 style={{ marginTop: 22 }}>Narration Library</h4>
          {assets.map((asset, index) => (
            <div key={asset.narration_id}>
            {(index === 0 || (assets[index - 1].category || "INDIVIDUAL_GAME") !== (asset.category || "INDIVIDUAL_GAME")) && (
              <h4 style={{ marginTop: 20, color: "#4f46e5", letterSpacing: 1 }} data-testid={`voice-category-${asset.category || "INDIVIDUAL_GAME"}`}>
                {asset.category || "INDIVIDUAL_GAME"}
              </h4>
            )}
            <div style={{ marginTop: 12, borderTop: "1px solid #eee", paddingTop: 10 }} data-testid={`voice-asset-${asset.narration_id}`}>
              ...
              {asset.kind === "static" && asset[settings.voice_environment]?.status === "ready" && (
              ...
              <div style={{ display: "flex", gap: 14, marginTop: 6, flexWrap: "wrap", alignItems: "center" }}>
                ...
                {asset.kind === "static" && (asset.test.status === "ready" || asset.live.status === "ready") && (
                  <a className="button button-small" href={`${API}/admin/game/voice/assets/${asset.narration_id}/download`}
                    data-testid={`voice-download-${asset.narration_id}`}>Download Audio</a>
                )}
              </div>
            </div>
            </div>
