...
export default function RecruitmentHomePage({ form }) {
  const video = useFlowVideo("board_recruitment_offer");
  return (
    <div className="bfg recruit-home-shell">
      ...
      <main className="recruit-home" data-testid="recruitment-landing-page">
        ...
        {video?.youtube_id && (
          <section className="recruit-section recruit-video" aria-labelledby="recruit-video-heading">
            <div className="recruit-section-heading">
              <p className="bfg-eyebrow">{copy.videoEyebrow}</p>
              <h2 id="recruit-video-heading">{copy.videoHeading}</h2>
              <p>{copy.videoText}</p>
            </div>
            <div className="bfg-video-frame">
              <iframe src={`https://www.youtube.com/embed/${video.youtube_id}`} title={copy.videoTitle}
