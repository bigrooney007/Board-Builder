...
export default function StrategicLeadershipPortfolioPage(){
 ...
 return <main className="legal-page" style={{maxWidth:820,margin:"0 auto",padding:"40px 18px"}}><p className="eyebrow">{data.organization_name}</p><h1>{data.title}</h1><p><strong>{data.member_name}</strong></p><pre style={{whiteSpace:"pre-wrap",fontFamily:"Georgia, serif",lineHeight:1.7}}>{data.display_text}</pre><a className="button" href={`${API}/guided/strategic-planning/leadership-portfolio/${token}/pdf`}><Download size={16}/> DOWNLOAD PORTFOLIO</a></main>
}