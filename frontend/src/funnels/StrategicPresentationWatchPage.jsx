...
export default function StrategicPresentationWatchPage(){
 ...
 return <BfgShell><main className="guided-page sp-dashboard"><section className="guided-section"><p className="bfg-eyebrow">LIVE STRATEGIC PLAN PRESENTATION</p>{error?<p className="bfg-error">{error}</p>:!data?<p>Joining the meeting…</p>:<><h1>{data.organization_name}</h1><p>Section {data.current_area_index+1} of {data.total_areas}. This screen follows the meeting facilitator automatically.</p>{data.area?<div className="sp-contentbox"><h2>{data.area.area}</h2>{data.area.owner_name&&<p><strong>Presenter:</strong> {data.area.owner_name}</p>}<p style={{whiteSpace:"pre-wrap"}}>{data.area.submitted_plan||data.area.direction}</p></div>:<p>The facilitator is preparing the first section.</p>}</>}</section></main></BfgShell>;
}
