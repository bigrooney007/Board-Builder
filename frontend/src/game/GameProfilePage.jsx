...
const STEPS = ["Your Organisation", "Your Fundraising Goal", "Primary User", "Review & Unlock"];
...
export default function GameProfilePage() {
  ...
  const unlock = async () => {
    ...
    try {
      await memberApi.put("/game/profile", { organization: org, goal: { ...goal, amount: goalAmount }, primary_user: user });
      const response = await memberApi.post("/payments/game-checkout", { origin_url: window.location.origin, cancel_path: "/game/start" });
        ...
        <div className="bfg-card">
          {step === 0 && (
            <div data-testid="bfg-profile-step-org">
              <p className="bfg-eyebrow">Your Fundraising Game Profile</p>
          ...
          {step === 1 && (
            <div data-testid="bfg-profile-step-goal">
              ...
              <Field label="Target date / deadline" type="date" value={goal.deadline} onChange={setGoalField("deadline")} testId="bfg-goal-deadline" />
              ...
              <Field label="Why does this fundraising goal matter now?" value={goal.why_now} onChange={setGoalField("why_now")} testId="bfg-goal-why-now" textarea />
            </div>
          )}
          {step === 2 && (
          ...
          {step === 3 && (
            <div data-testid="bfg-profile-review">
              <p className="bfg-eyebrow">Review</p>
              <h2>Your Fundraising Game</h2>
              ...
              <button className="bfg-btn bfg-btn-ghost bfg-btn-sm" onClick={() => setStep(0)} data-testid="bfg-edit-profile-btn">Edit Profile</button>
              <div className="bfg-price-card" style={{ marginTop: 30 }}>
                <h2 style={{ fontSize: 22 }}>UNLOCK YOUR BOARD FUNDRAISING GAME</h2>
                <p className="bfg-price-amount" data-testid="bfg-unlock-price">{content.price_display}<small>One-Time Payment — {content.price_note}</small></p>
                <button className="bfg-btn bfg-btn-primary" onClick={unlock} disabled={busy} data-testid="bfg-unlock-btn">
                  {busy ? "Preparing secure checkout…" : `Unlock My Game — ${content.price_display}`}
                </button>
              </div>
              <div className="bfg-benefits" style={{ marginTop: 26 }}>
          ...
          {step < 3 && (
            <div className="bfg-form-actions">
              ...
              <button className="bfg-btn bfg-btn-primary" onClick={nextStep} disabled={busy} data-testid="bfg-profile-next">
                {busy ? "Saving…" : step === 2 ? "Review My Game" : "Continue"}
              </button>
            </div>
          )}
        </div>
